...
class GraphCNNWithRNNExperiment(GraphCNNExperiment):
    def preprocess_data(self, dataset):
        ...
        # self.min_num_file = dataset[0]
        # self.max_num_file = dataset[1]
        # self.root_dir = dataset[2]
        

        # self.no_samples = self.max_num_file - self.min_num_file
    ...
    def create_data(self):
        ...
        graph_size = np.array([s[0] for s in meta[1]]).astype(np.int64)
        ...
        with tf.device("/cpu:0"):
            with tf.variable_scope('input') as scope:
                ...
                with tf.variable_scope('train_data') as scope:
                    ...
                    training_samples = [np.array(vertex),np.array(adj),np.array(labels), np.array(meta[0]),np.array(masks)]
                    training_samples = self.create_input_variable(training_samples)
                    ...
                    single_sample[0].set_shape([400,largest_graph,30,largest_graph])
                    single_sample[1].set_shape([400,largest_graph,largest_graph])
                ...
                with tf.variable_scope('test_data') as scope:
                    ...
                    single_sample[0].set_shape([100,largest_graph,30,largest_graph])
                    single_sample[1].set_shape([100,largest_graph,largest_graph])
    ...
    def print_sample(self,sample):
        sample = [self.i_to_word[i] if i in self.i_to_word and i != 0 else "" for i in sample]
        return " ".join(sample)
